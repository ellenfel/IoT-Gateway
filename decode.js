...
function decodeSensorData(bytes) {
    ...
    while (buffer.remaining() > 0) {
        ...
        if (channel_id === 0x01 && channel_type === 0x75) {
            decoded.battery = buffer.readUInt8().toString();
        }
        // TEMPERATURE
        else if (channel_id === 0x03 && channel_type === 0x67) {
            decoded.temperature = (buffer.readUInt16LE() / 10).toString();
        }
        // DISTANCE
        else if (channel_id === 0x04 && channel_type === 0x82) {
            decoded.distance = buffer.readUInt16LE().toString();
        }
        // POSITION
...
var bytes = readHexString(process.argv[2]);
var payload = decodePayload(bytes);

console.log(JSON.stringify(payload.data).replace(/\s/g, ''));
